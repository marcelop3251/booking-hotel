...
export const CheckInAdmin = () => { 

    const [booking, setBooking] = useState([])

    const onClicked = async (e) => { 
        try { 
            const { data } = await HotelService.approveCheck("check-in", e.id)
    ...
    const fetchBooking = async () => { 
        try {
            const { data } = await HotelService.getAllBookingAdmin("CHECK_IN")
            setBooking(data)
        } catch (error) { 
            console.log(error)
        }
    ...
    return ( 
        <div>
            ...
            <div class="container container-admin">
                <h1>Clientes aguardando confirmação de check in</h1>
